...
As our working example, we will be refactoring a simple piece of code that uses a `defaultOwner` global variable so the callers can use its accessors instead of manipulating it directly.
...
There are two tests suites for this example. The first one covers the getter and setter functions for the `defaultOwner` variable, and the second one covers the basic configuration for the `Person` class that will be introduced later on the refactoring steps.

### Steps

To start with this refactoring, we need to create indirection to protect callers of directly accessing the `defaultOwner` variable. We can do that by creating getter and setter functions to return and manipulate the variable's data, and update the callers to use these accessors instead of touching the variable directly:
...
After we've done that, we can move the `defaultOwner` variable and its accessors to their own file and update the callers:
...
What about nested object properties, though? This is the main gotcha when it comes to `Object.assign`: it creates shallow copies of the properties in a given object, but still keep pointer references to the original object structure in the Heap when it's a complex object, so we can run into the same problem even when using it as a partial solution.
...
But it's Javascript, and as we all know, there are other paths to Asgard. A really diabolic developer could access the private `_firstName` variable directly and change its value:

```javascript
owner._firstName = 'Morpheus';
```

And we can only protect against this kind of threads with good politics and thorough code reviews.
...
The following chart shows how allocation will work for these assignments above:
...
Note how each `employee*` variable has its own pointer in the Stack, but these pointers all targets the same entry in the Heap. Which means that any changes to the data structure will be replicated to all the pointers holding a reference to it.
